...
type
  TForm12 = class(TForm)
    ...
    procedure Rectangle2MouseUp(Sender: TObject; Button: TMouseButton;
      Shift: TShiftState; X, Y: Single);
    procedure ПроверитьClick(Sender: TObject);
...
var
  Form12: TForm12; bool1,bool2,bool3,bool4,bool5,bool6 : boolean; xx1,yy1 : Single; s:integer;

implementation

{$R *.fmx}
  uses Unit3, Unit13, Unit11;
...
MediaPlayer1.FileName:='file:///storage/emulated/0/Download/audio/заданиеалфавит.mp3';
MediaPlayer1.Play;
 end
else  begin
...
bool6:=false;
 ...
 for i:=1 to 5 do begin randomize;
  ...
  a[k]:=0;                                     //рандом расположения кубиков
  end;
  if i=2 then
  begin
...
procedure TForm12.FormKeyUp(Sender: TObject; var Key: Word; var KeyChar: Char;
  Shift: TShiftState);
begin
...
procedure TForm12.Rectangle2MouseDown(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Single);
begin
   ...
   end;                                                                                                                                       //последовательность важна, так как СЛОИ
...
procedure TForm12.Rectangle2MouseMove(Sender: TObject; Shift: TShiftState; X,
  Y: Single);
begin
     if (bool5=True) then begin
 if Rectangle8.Position.Y<1 then Rectangle8.Position.Y:=Rectangle8.Position.Y+1;
 if Rectangle8.Position.Y>159 then  Rectangle8.Position.Y:=Rectangle8.Position.Y-1; //чтобы компонент не застрял
 ...
 if (x-xx1<1) or (x-xx1>327)   // ограничения
 or (y-yy1<1) or (y-yy1>159)
 then
  Exit
 else begin
 rectangle8.Position.X:=x-xx1;
 rectangle8.Position.Y:=y-yy1;      //двигает компонент, следуя за указателем
end;      end;

 if (bool4=True) then begin
 if Rectangle7.Position.Y<1 then Rectangle7.Position.Y:=Rectangle7.Position.Y+1;
 if Rectangle7.Position.Y>159 then  Rectangle7.Position.Y:=Rectangle7.Position.Y-1; //чтобы компонент не застрял
 ...
 rectangle7.Position.Y:=y-yy1;      //двигает компонент, следуя за указателем
end;      end;

if (bool3=True) then begin
 if Rectangle6.Position.Y<1 then Rectangle6.Position.Y:=Rectangle6.Position.Y+1;
 if Rectangle6.Position.Y>159 then  Rectangle6.Position.Y:=Rectangle6.Position.Y-1; //чтобы компонент не застрял
 ...
 rectangle6.Position.Y:=y-yy1;      //двигает компонент, следуя за указателем
end;      end;

if (bool2=True) then begin
 if Rectangle5.Position.Y<1 then Rectangle5.Position.Y:=Rectangle5.Position.Y+1;
 if Rectangle5.Position.Y>159 then  Rectangle5.Position.Y:=Rectangle5.Position.Y-1; //чтобы компонент не застрял
 ...
 rectangle5.Position.Y:=y-yy1;      //двигает компонент, следуя за указателем
end;      end;

if (bool1=True) then begin
 if Rectangle4.Position.Y<1 then Rectangle4.Position.Y:=Rectangle4.Position.Y+1;
 if Rectangle4.Position.Y>159 then  Rectangle4.Position.Y:=Rectangle4.Position.Y-1; //чтобы компонент не застрял
 ...
 rectangle4.Position.Y:=y-yy1;      //двигает компонент, следуя за указателем
end;      end;
end;

procedure TForm12.Rectangle2MouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Single);
begin
    bool5:=false;
    bool4:=false;
...
s:=s+1;
 ...
 if s=1 then
  begin
   rectangle26.visible:=true;  Application.CreateForm(TForm13, Form13);
...
procedure TForm12.ПроверитьClick(Sender: TObject);
begin
...
MediaPlayer1.Play;
end else begin showmessage('Допущена ошибка');
